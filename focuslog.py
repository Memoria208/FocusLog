...
def show_dashboard(root):
    ...
    fig.suptitle("FocusLog Dashboard", fontsize=16, fontweight="bold")
...
def main_menu(root, state=None):
    ...
    if state:
        tk.Button(
            ...
            fg="white",
            bg="firebrick",
            command=lambda: [win.destroy(), stop_session(root, state)]
        ).pack(pady=5)
